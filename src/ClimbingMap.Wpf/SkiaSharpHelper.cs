...
namespace ClimbingMap.Wpf {
   public static class SkiaSharpHelper {
      ...
      internal static void DrawEllipse(SKCanvas canvas, Point center, Point radius, Color color) {
         canvas.DrawOval(
            (float) center.X,
            (float) center.Y,
            (float) Math.Abs(center.X - radius.X),
            (float) Math.Abs(center.Y - radius.Y),
            GetPaint(color.ToSkColor()));
      }
      ...
      /// <param name="scaleFactor">Scale factor if scaling is to be made</param>
      /// <returns></returns>
      public static SKPoint ToSKPoint(this Point point, Point? originPoint = null, double scaleFactor = 1) {
      ...
      public static Point ToPoint(this SKPoint self) {
